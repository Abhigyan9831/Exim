...
export function HeroSection({
  ...
  subtitle = "From the Eastern Himalayas to your kitchen",
  image = "/images/her.webp"
}: HeroProps) {
  return (
    <section className="relative min-h-[100dvh] md:min-h-[400px] md:aspect-[1647/955] md:max-h-[calc(100vh-80px)] flex items-center justify-center overflow-hidden w-full">
      ...
      <div className="relative z-20 text-center text-white max-w-4xl mx-auto px-4 -mt-16 md:-mt-12">
        <h1 className="text-4xl md:text-6xl font-bold mb-4 md:mb-6 whitespace-pre-line">
          {title}
        </h1>
    ...
    <section className="py-8 md:py-12 bg-[#030303]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl md:text-4xl font-bold text-white mb-6 md:mb-10 text-center">
          {title.includes("M R EXIM") ? (
            <>
              {title.split("M R EXIM")[0]}
              <span className="text-[#A35C10]">M R EXIM</span>
              {title.split("M R EXIM")[1]}
            </>
          ) : (
            title
          )}
...
export function WhyChooseSection({
  title = "Why Choose M R EXIM?",
  subtitle = "Connecting Indian spices with global markets"
}: WhyChooseProps) {
  const features = [
    {
      icon: (
    ...
    },
    {
      ...
      featureTitle: "Quality Assurance",
    ...
    },
    {
      ...
      featureTitle: "Eco-Friendly Cultivation",
...
export function OurProductSection({
  title = "Our Product",
  productName = "Himalayan Black Cardamom",
  image1 = "/images/ChatGPT Image May 19, 2026, 11_39_19 PM.webp",
  image2 = "/images/ChatGPT Image May 23, 2026, 01_23_08 AM.webp"
}: OurProductProps) {
  return (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl md:text-4xl font-bold mb-8 md:mb-12 text-center">
          {title.includes("Product") ? (
            <>
              {title.split("Product")[0]}
              <span className="text-[#A35C10]">Product</span>
              {title.split("Product")[1]}
            </>
          ) : (
            title
          )}
...
interface ReadyToPartnerProps {
  title?: string;
  subtitle?: string;
}

export function ReadyToPartnerSection({
  title = "Ready to Partner With Us?",
  subtitle = "Get in touch today to discuss your specialty agricultural commodity needs"
}: ReadyToPartnerProps) {
  return (
    <section className="py-3 md:py-4 pb-12 md:pb-16 bg-brand-grey shadow-inner">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="text-2xl md:text-4xl font-bold text-white mb-4 md:mb-6">
          {title.includes("With Us") ? (
            <>
              {title.split("With Us")[0]}
              <span className="text-[#A35C10]">With Us</span>
              {title.split("With Us")[1]}
            </>
          ) : (
            title
          )}
